...
ALLOW_MANAGED_CONFIG_OVERWRITE="${ALLOW_MANAGED_CONFIG_OVERWRITE:-0}"
case "${ALLOW_MANAGED_CONFIG_OVERWRITE}" in
  0|1) ;;
  *) echo "ALLOW_MANAGED_CONFIG_OVERWRITE must be 0 or 1. Current value=${ALLOW_MANAGED_CONFIG_OVERWRITE}."; exit 1 ;;
esac
...
is_managed_modprobe_file() {
  local path="$1"
  [[ -f "${path}" && ! -L "${path}" ]] || return 1
  awk '
    NR == 1 && $0 ~ /^options nf_conntrack hashsize=[1-9][0-9]*$/ { valid = 1; next }
    { exit 1 }
    END { if (NR != 1 || !valid) exit 1 }
  ' "${path}"
}

is_managed_modules_file() {
  local path="$1"
  [[ -f "${path}" && ! -L "${path}" ]] || return 1
  awk '
    NR == 1 && $0 == "tcp_bbr" { next }
    NR == 2 && $0 == "sch_fq" { next }
    NR == 3 && $0 == "nf_conntrack" { valid = 1; next }
    { exit 1 }
    END { if (NR != 3 || !valid) exit 1 }
  ' "${path}"
}

is_managed_limits_dropin() {
  local path="$1"
  local nofile
  local -a lines=()
  [[ -f "${path}" && ! -L "${path}" ]] || return 1
  ...
  (( ${#lines[@]} == 4 )) || return 1
  [[ "${lines[0]}" =~ ^\*[[:space:]]+soft[[:space:]]+nofile[[:space:]]+([1-9][0-9]*)$ ]] || return 1
  nofile="${BASH_REMATCH[1]}"
  [[ "${lines[1]}" =~ ^\*[[:space:]]+hard[[:space:]]+nofile[[:space:]]+${nofile}$ ]] || return 1
  [[ "${lines[2]}" =~ ^root[[:space:]]+soft[[:space:]]+nofile[[:space:]]+${nofile}$ ]] || return 1
  [[ "${lines[3]}" =~ ^root[[:space:]]+hard[[:space:]]+nofile[[:space:]]+${nofile}$ ]]
}

is_managed_manager_limits() {
  local path="$1"
  local nofile
  local -a lines=()
  [[ -f "${path}" && ! -L "${path}" ]] || return 1
  ...
  [[ "${lines[0]}" == "[Manager]" ]] || return 1
  [[ "${lines[1]}" =~ ^DefaultLimitNOFILE=([1-9][0-9]*)$ ]] || return 1
  nofile="${BASH_REMATCH[1]}"
  if (( ${#lines[@]} == 3 )); then
    [[ "${lines[2]}" == "DefaultLimitNPROC=${nofile}" ]] || return 1
  fi
}

is_managed_profile_limits() {
  local path="$1"
  [[ -f "${path}" && ! -L "${path}" ]] || return 1
  awk '
    NR == 1 && $0 ~ /^ulimit -SHn [1-9][0-9]* 2>\/dev\/null \|\| true$/ { valid = 1; next }
    { exit 1 }
    END { if (NR != 1 || !valid) exit 1 }
  ' "${path}"
}

is_managed_sysctl_dropin() {
  local path="$1"
  local first_line=""
  [[ -f "${path}" && ! -L "${path}" ]] || return 1
  IFS= read -r first_line < "${path}" || true
  [[ "${first_line}" == "# Debian / Ubuntu userspace TCP/UDP relay and proxy endpoint optimization." ]]
}

is_managed_sysctl_default() {
  local path="$1"
  local -a lines=()
  [[ -f "${path}" && ! -L "${path}" ]] || return 1
  mapfile -t lines < "${path}"
  (( ${#lines[@]} == 2 || ${#lines[@]} == 3 )) || return 1
  [[ "${lines[0]}" =~ ^NF_CONNTRACK_HASH_SIZE=[1-9][0-9]*$ ]] || return 1
  [[ "${lines[1]}" =~ ^NETDEV_BUDGET_USECS=[1-9][0-9]*$ ]] || return 1
  if (( ${#lines[@]} == 3 )); then
    [[ "${lines[2]}" =~ ^IPFRAG_HIGH_THRESH=[1-9][0-9]*$ ]] || return 1
  fi
}

is_managed_network_tune_default() {
  local path="$1"
  local -a lines=()
  [[ -f "${path}" && ! -L "${path}" ]] || return 1
  mapfile -t lines < "${path}"
  (( ${#lines[@]} == 2 || ${#lines[@]} == 4 )) || return 1
  [[ "${lines[0]}" =~ ^TXQUEUELEN=[1-9][0-9]*$ ]] || return 1
  [[ "${lines[1]}" =~ ^RPS_FLOW_ENTRIES=[1-9][0-9]*$ ]] || return 1
  if (( ${#lines[@]} == 4 )); then
    [[ "${lines[2]}" =~ ^RP_FILTER=[012]$ ]] || return 1
    [[ "${lines[3]}" =~ ^MAXIMIZE_NIC_RING=[01]$ ]] || return 1
  fi
}
...
preflight_managed_file() {
  ...
  if [[ ! -f "${path}" || -L "${path}" ]]; then
    echo "Refusing to replace a non-regular or symbolic-link managed path: ${path}."
    exit 1
  fi
  if ! "${validator}" "${path}" && [[ "${ALLOW_MANAGED_CONFIG_OVERWRITE}" != "1" ]]; then
    echo "Refusing to replace a same-name file not generated by the legacy script or main2: ${path}."
    exit 1
  fi
}

preflight_managed_paths() {
  preflight_managed_file /etc/modprobe.d/99-network-optimization.conf is_managed_modprobe_file
  preflight_managed_file /etc/modules-load.d/99-network-optimization.conf is_managed_modules_file
  preflight_managed_file "${LIMITS_FILE}" is_managed_limits_dropin
  preflight_managed_file /etc/systemd/system.conf.d/99-limits.conf is_managed_manager_limits
  preflight_managed_file /etc/systemd/user.conf.d/99-limits.conf is_managed_manager_limits
  preflight_managed_file /etc/profile.d/99-ulimit.sh is_managed_profile_limits
  if [[ ! -L "${SYSCTL_FILE}" ]]; then
    preflight_managed_file "${SYSCTL_FILE}" is_managed_sysctl_dropin
  fi
  preflight_managed_file "${SYSCTL_DEFAULT_FILE}" is_managed_sysctl_default

  if [[ -e "${SYSCTL_APPLY_SCRIPT}" || -L "${SYSCTL_APPLY_SCRIPT}" ]]; then
    if [[ ! -f "${SYSCTL_APPLY_SCRIPT}" || -L "${SYSCTL_APPLY_SCRIPT}" ]]; then
      echo "Refusing to replace a non-regular or symbolic-link managed path: ${SYSCTL_APPLY_SCRIPT}."
      exit 1
    fi
    if [[ "${ALLOW_MANAGED_CONFIG_OVERWRITE}" != "1" ]] &&
       ! file_sha256_is "${SYSCTL_APPLY_SCRIPT}" bc18c4ec894173bc0d696ab41982b2ff232f0d886c05a14b22dd7a89b6f36d06 &&
       ! file_sha256_is "${SYSCTL_APPLY_SCRIPT}" e96cb7ec66a59f8d98b22b34bf966aab58468a72a72e382d8ea80cde10b5e411 &&
       ! file_sha256_is "${SYSCTL_APPLY_SCRIPT}" 999c4866911e98c7a61e6f7ba98d175d5224f4dcca93a251b8a0ced10ceb029b; then
      echo "Refusing to replace a same-name script not generated by main2: ${SYSCTL_APPLY_SCRIPT}."
      exit 1
    fi
  fi
  if [[ -e "${SYSCTL_SERVICE_FILE}" || -L "${SYSCTL_SERVICE_FILE}" ]]; then
    if [[ ! -f "${SYSCTL_SERVICE_FILE}" || -L "${SYSCTL_SERVICE_FILE}" ]]; then
      echo "Refusing to replace a non-regular or symbolic-link managed path: ${SYSCTL_SERVICE_FILE}."
      exit 1
    fi
    if [[ "${ALLOW_MANAGED_CONFIG_OVERWRITE}" != "1" ]] &&
       ! file_sha256_is "${SYSCTL_SERVICE_FILE}" f79cd486003bd9352ed7653404c2277d2ce511c71e32bab571122969e948293d; then
      echo "Refusing to replace a same-name service not generated by main2: ${SYSCTL_SERVICE_FILE}."
      exit 1
    fi
  fi
}
...
preflight_network_tune_paths() {
  ...
  for path in "${NETWORK_TUNE_DEFAULT_FILE}" "${NETWORK_TUNE_SCRIPT}" "${NETWORK_TUNE_SERVICE_FILE}"; do
    if [[ ( -e "${path}" || -L "${path}" ) && ( ! -f "${path}" || -L "${path}" ) ]]; then
      echo "Refusing to replace non-regular file ${path}."
      exit 1
    fi
  done
  if [[ -e "${NETWORK_TUNE_DEFAULT_FILE}" ]] &&
     ! is_managed_network_tune_default "${NETWORK_TUNE_DEFAULT_FILE}" &&
     [[ "${ALLOW_MANAGED_CONFIG_OVERWRITE}" != "1" ]]; then
    echo "Refusing to replace a same-name network tuning config not generated by the legacy script or main2: ${NETWORK_TUNE_DEFAULT_FILE}."
    exit 1
  fi
  ...
  if [[ "${ALLOW_MANAGED_CONFIG_OVERWRITE}" == "1" ]]; then
    return 0
  fi
...
backup_file() {
  ...
  declare -gA MAIN2_BACKED_UP_PATHS
  if [[ -n "${MAIN2_BACKED_UP_PATHS["${path}"]+x}" ]]; then
    return 0
  fi
  if [[ -e "${path}" ]]; then
    ...
    while [[ -e "${backup_path}" || -L "${backup_path}" ]]; do
      counter=$((counter + 1))
      backup_path="${path}.bak.${BACKUP_SUFFIX}.${counter}"
    done
    if ! cp -a -- "${path}" "${backup_path}"; then
      echo "Failed to back up managed path before overwrite: ${path}." >&2
      return 1
    fi
    MAIN2_BACKED_UP_PATHS["${path}"]=1
  fi
}
...
backup_managed_paths_for_explicit_overwrite() {
  ...
  [[ "${ALLOW_MANAGED_CONFIG_OVERWRITE}" == "1" ]] || return 0
...
migrate_legacy_sysctl() {
  ...
  awk '
    BEGIN {
      ...
      managed["net.ipv4.ipfrag_high_thresh"] = 1
      managed["net.ipv4.ip_local_port_range"] = 1
      managed["net.core.netdev_budget_usecs"] = 1
      managed["net.ipv6.conf.all.accept_redirects"] = 1
      managed["net.ipv6.conf.default.accept_redirects"] = 1
      managed["net.ipv6.conf.all.accept_source_route"] = 1
      managed["net.ipv6.conf.default.accept_source_route"] = 1
      managed["net.ipv6.ip_nonlocal_bind"] = 1
      managed["net.ipv6.ip6frag_high_thresh"] = 1
      managed["net.ipv6.ip6frag_low_thresh"] = 1
      managed["net.ipv6.ip6frag_time"] = 1
      managed["net.ipv6.neigh.default.gc_thresh1"] = 1
      managed["net.ipv6.neigh.default.gc_thresh2"] = 1
      managed["net.ipv6.neigh.default.gc_thresh3"] = 1
    }
    NR == FNR {
      line = $0
      sub(/^[[:space:]]+/, "", line)
...
for numeric_name in NETDEV_BUDGET_USECS IPFRAG_HIGH_THRESH; do
  if (( 10#${!numeric_name} > 2147483647 )); then
    echo "${numeric_name} must not exceed 2147483647. Current value=${!numeric_name}."
    exit 1
  fi
done
...
: "${IPFRAG_HIGH_THRESH:?Missing IPFRAG_HIGH_THRESH in ${SYSCTL_DEFAULT_FILE}}"

for numeric_name in NETDEV_BUDGET_USECS IPFRAG_HIGH_THRESH; do
  numeric_value="${!numeric_name}"
  if [[ ! "${numeric_value}" =~ ^[1-9][0-9]{0,9}$ ]] ||
     (( 10#${numeric_value} > 2147483647 )); then
    echo "Invalid ${numeric_name}=${numeric_value} in ${SYSCTL_DEFAULT_FILE}." >&2
    exit 1
  fi
done
...
apply_optional_numeric_sysctl() {
  local key="$1"
  local requested="$2"
  local proc_file="/proc/sys/${key//./\/}"
  ...
  if write_error="$(sysctl -q -w "${key}=${requested}" 2>&1)"; then
    actual="$(<"${proc_file}")"
    if [[ "${actual}" != "${requested}" ]]; then
      echo "Failed to verify ${key}: expected=${requested} actual=${actual:-unavailable}." >&2
      return 1
    fi
    return 0
  fi
  ...
  if [[ "${actual}" != "${current}" ]]; then
    echo "Failed to preserve ${key} after the kernel rejected ${requested}: before=${current} actual=${actual:-unavailable}." >&2
    return 1
  fi
  echo "Warning: failed to apply ${key}=${requested}; keeping current=${current}. ${write_error}" >&2
}

sysctl -e -p "${SYSCTL_FILE}"
apply_optional_numeric_sysctl net.core.netdev_budget_usecs "${NETDEV_BUDGET_USECS}"
apply_optional_numeric_sysctl net.ipv4.ipfrag_high_thresh "${IPFRAG_HIGH_THRESH}"
...
diagnose_network_sysctl_service_failure() {
  echo "network-optimization-sysctl.service failed; showing diagnostics." >&2
  systemctl status network-optimization-sysctl.service --no-pager --full >&2 || true
  journalctl -u network-optimization-sysctl.service -n 120 --no-pager >&2 || true
}

start_network_sysctl_service() {
  if systemctl restart network-optimization-sysctl.service &&
     systemctl is-active --quiet network-optimization-sysctl.service; then
    return 0
  fi
  if ! systemctl reset-failed network-optimization-sysctl.service; then
    diagnose_network_sysctl_service_failure
    return 1
  fi
  if systemctl restart network-optimization-sysctl.service &&
